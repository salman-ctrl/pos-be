...
exports.initiateLogin = async (req, res) => {
  try {
    ...
    try {
        ...
        return res.status(500).json({ success: false, message: "Gagal mengirim email OTP. Cek konfigurasi server." });
    }

    res.json({ success: true, message: "OTP dikirim ke email Anda." });
...
exports.verifyOtp = async (req, res) => {
    try {
        ...
        if (new Date() > new Date(user.otpExpiresAt)) {
            return res.status(400).json({ success: false, message: "Kode OTP kadaluarsa. Silakan login ulang." });
        }
